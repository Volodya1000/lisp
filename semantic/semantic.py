...
# Набор встроенных функций (можешь дополнить)
BUILTIN_FUNCTIONS = {
    "+": {"param_count": None},  # variadic
    "-": {"param_count": None},
    "*": {"param_count": None},
    "/": {"param_count": 2},  # для упрощения считаем бинарной (можно сделать variadic)
    "car": {"param_count": 1},
    "cdr": {"param_count": 1},
    "cons": {"param_count": 2},
...
class SemanticVisitor(ANTLRBaseVisitor):
    ...
    def visitProgram(self, ctx):
        ...
        for expr in exprs:
            try:
                ...
                self._add_error(e)
            # Если возникла другая ошибка (TypeError, etc.), пусть она всплывет
        return None

    # 🛑 МЕТОД visitExpr УДАЛЕН 🛑
    # ANTLR-овский self.visit(expr) сам вызовет visitAtom, visitList и т.д.
    ...
    # 🛑 МЕТОД visitBinding УДАЛЕН 🛑
    # Логика перенесена в visitLetExpr, т.к. binding'и обрабатываются
    # в двух разных scope (выражения в старом, переменные в новом)
    ...
    # 🛑 МЕТОД visitChildren УДАЛЕН 🛑
    # Мы используем реализацию по умолчанию из ANTLRBaseVisitor
